...
class ConfigList(object):
    ...
    def __init__(self,config, root_name='', group_keys_val='',group_keys=[]):
        if 'hierarchical' in config[0]:
            ...
            self.config = [{'hierarchical' : r, 'flattened': flatten_dict(r,parent_key=root_name) } for r in config ]
    ...
    def get_data(self,keys):
        ...
        out = {key: self.config[0]['flattened'][key] for key in keys if not key in protected  }
        if "group_keys_val" in keys:
            out["group_keys_val"] = self.group_keys_val
        if "group_keys" in keys:
            key_name = '-'.join(['-'.join([el.split('.')[-1] for el in key]) for key in self.group_keys])
...
class ConfigCollection(object):
    ...
    def groupBy(self,list_group_keys=[]):
        ...
        groups = {key:ConfigList(value).groupBy(list_group_keys) for key, value in val_dict.items()}
...
def aggregate(collection_dict,aggregation_maps):
    ...
    if not isinstance(collection_dict, ConfigCollection):
        ...
        agg_dict = AggregatedConfigs({key:GroupedConfigs(value) for key,value in agg_dict.items()}) 
    else:
...
def aggregate_collection(collection_list,agg_maps):
    ...
    for collection in collection_list:
        ...
        for agg_map in agg_maps:
            ...
            config = [deepcopy(collection.config[index])] if index>=0 else deepcopy(collection.config)
            for config_dict in config:
                config_dict['flattened'].update(agg_val) 
            
            collection = ConfigList(config, 
...
def GroupedConfigs_to_ConfigList(collection):
    assert isinstance(collection, GroupedConfigs) or isinstance(collection, ConfigCollection)
    list_config = []
