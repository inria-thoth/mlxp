...
class DataFrame(list):
    ...
    def groupby(self, group_keys: Union[str,List[str]]) -> GroupedDataFrame:
        ...
        :params group_keys: A string or list of strings containing the names of the columns
            to be grouped.
        :type group_keys: Union[str,List[str]]
        :return: A dictionary of dataframes grouped by the values of the columns provided
            to group_keys. Each key of the dictionary is a tuple of values taken by the columns in group_keys.
    ...
    def sort(self, by:Union[str,List[str]], ascending:bool=True)->DataFrame:
        ...
        :params by: Either column name or a list of column names by which the dataframe must be sorted with.
        :type maps: Union[str,List[str]]
        :params ascending: Sorting either by increasing values (ascending=True) or descreasing values (ascending=False) of the specified columns. 
        :type ascending: bool
...
class GroupedDataFrame:
    ...
    def ungroup(self)-> DataFrame:
        ...
        :return: A dataframe representing the ungrouped version of the groupped dictionary.
        :rtype: DataFrame        
        """
        data_list = []
        try: 
    ...
    def toPandas(self, lazy=True) -> pd.DataFrame:
        ...
        :param: If true the pandas dataframe does not contain the results of data
            loaded lazyly.
        :return: A panda dataframe containing logs (configs and data) of the
            DataFrame object
        :rtype: pd.DataFrame
        ...
        if lazy:
            if self.pandas_lazy is None:
                self.pandas_lazy = _groups_toPandas(self.grouped_dict,
                                               self.group_keys,
                                               True)
            return self.pandas_lazy
        else:
            if self.pandas is None:
                self.pandas = _groups_toPandas(self.grouped_dict,
                                   self.group_keys,
                                   False)
            return self.pandas
    def _apply_to_groups(self, maps, operation, kwargs, ungroup: bool=False)->Union[GroupedDataFrame,DataFrame]:
        grouped_res = {}
        for keys, config_list in self.items():
    ...
    def apply(self,maps: Union[Map, List[Map]], map_type='Generic',  ungroup: bool=False)-> GroupedDataFrame:
        """Applies a generic tranformation to each dataframe representing each group. see DataDictsList.apply.
        ...
        :params maps: Either a single instance of tuple Map or a list of tuple of type Map. 
        :type maps: Union[Map, List[Map]]
        :params map_type: Type of the transformation to apply (see DataDictsList.apply): 'Generic', 'Columnwise', 'Rowwise' or 'Pointwise'.
        :type map_type: str
    ...
    def aggregate(self, maps: Union[Map, List[Map]], ungroup: bool=False) -> Union[GroupedDataFrame,DataFrame]:
        """Perform aggregation of the dataframe corresponding to each group. see DataDictsList.aggregate.
        Returns a groupped dataframe of type GroupedDataFrame that is optionally ungroupped into a dataframe object of type DataFrame.
    ...
    def map(self,maps: Union[Map, List[Map]], ungroup: bool=False)-> Union[GroupedDataFrame,DataFrame]:
        """Applies a pointwise tranformation to the dataframe corresponding to each group. see DataDictsList.map.
        Returns a groupped dataframe of type GroupedDataFrame that is optionally ungroupped into a dataframe object of type DataFrame.
    ...
    def sort(self, by: List[str], ascending:bool=True,ungroup:bool=False)->Union[GroupedDataFrame,DataFrame]:
        """Returns a grouped object where the dataframe of each group is sorted according to a list of columns. (see DataFrame.sort)
        optionally ungroupps the grouped object into a single dataframe of type DataFrame.
...
def _format_reducing(data_dict,dataframe_size):
    ...
    if not reducing[0]:
        outputs = [{key: value[i] for key,value in data_dict.items()} 
